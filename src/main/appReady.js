...
import appRegionOverlayCss from '../renderer/css/app-region-overlay.css?raw';
import { kioskSiteForHtmlUrl } from './kiosk-sites';
...
async function appReady(args) {
  ...
  if (args['disable-selection'])
    preloadModules.push(path.join(__dirname, '../preload/disableSelection.js'));

  if (args['disable-drag'])
    preloadModules.push(path.join(__dirname, '../preload/disableDrag.js'));

  if (args['hide-cursor'])
    preloadModules.push(path.join(__dirname, '../preload/hideCursor.js'));
  ...
  mainWindowSingleton.webContents.openDevTools();

  ipcMain.on('get-preload-modules-for-site', (event, url) => {
    const sitePreloadModules = [...preloadModules];
    try {
      const site = kioskSiteForHtmlUrl(url);
      sitePreloadModules.push(site.preload.pathname);
    } catch (e) {
      // Has no matching preload script. NOOP.
    }
    event.returnValue = sitePreloadModules;
  });
